...
public class ApiLogTopology {
	...
	private void startCluster() throws Exception {
		...
		config.put(Config.STORM_ZOOKEEPER_PORT,
				parseZkPort("192.168.156.58:2181,192.168.156.60:2181"));
		config.put(Config.STORM_ZOOKEEPER_SERVERS,
				parseZkHosts("192.168.156.58:2181,192.168.156.60:2181"));
...
//		builder.setBolt(Constants.STARTREQLOG, startReqLogBolt, 10)
//				.shuffleGrouping(Constants.KAFKASPOUT);
//		builder.setBolt(Constants.ENDREQLOG, endReqLogBolt, 10)
//				.shuffleGrouping(Constants.KAFKASPOUT);
//		builder.setBolt(Constants.REQLOG, reqLogBolt, 10)
//				.fieldsGrouping(Constants.STARTREQLOG,
//						new Fields(Constants.HOST, Constants.TRANSID))
//				.fieldsGrouping(Constants.ENDREQLOG, new Fields(Constants.HOST, Constants.TRANSID));
//		builder.setBolt(Constants.PRINTLOG + 2, printLogBolt).shuffleGrouping(Constants.REQLOG);
	...
	private TransLogBolt createTransLogBolt() {
		...
		bolt.setOutPutFieldNames(Constants.HOST, Constants.MODULE, Constants.DAY, Constants.TRANSLOG);
		return bolt;
	}
